...
<th {{ $attributes->merge(['class' => 'px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider'])->only('class') }}>
    ...
    @else
        <button
            {{ $attributes->except('class') }} class="flex items-center space-x-1 uppercase text-left text-xs leading-4 font-medium">
            <span>{{ $slot }}</span>
